...
function todoReducer(state, action) {
  switch (action.type) {
    ...
    case EDIT_TODO:
      ...
      updatedTodoList[foundedIndex] = Object.assign({}, updatedTodoList[foundedIndex], updatedTodo);
      return {
    ...
    case SEARCH_TODO:
      const searchTodoList = [...state.todos].filter((todo) => todo.task.toLowerCase().includes(action.payload.searchText.toLowerCase()));
      return {
        todos: [...state.todos],
    ...
    case SELECT_TODO_LIST:
      ...
      if (selectedIndex === 0) {
        ...
        const todayList = [...state.todos].filter((todo) => todo.date === today);
        return { todos: state.todos, todosFilter: todayList };
      } else if (selectedIndex === 2) {
        const sevenDayList = [...state.todos].filter((todo) => todo.date >= today && todo.date <= nextSevenDay);
